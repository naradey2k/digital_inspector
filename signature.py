...
for page_num, page in enumerate(pages, start=1):
    ...
    for i, bbox in enumerate(detections.xyxy):
        ...
        # Expand bbox to full width and add 100px to height
        expanded_x1 = 0
        expanded_x2 = image_width
        expanded_y1 = y1
        expanded_y2 = min(y2 + 100, image_height)  # Add 100px but don't exceed image bounds
        
        # Crop the expanded region
        cropped_region = image[expanded_y1:expanded_y2, expanded_x1:expanded_x2]
        
        # Apply OCR to extract text using EasyOCR
    ...
    cv2.imwrite(output_path, annotated_image)  # image is already in BGR format
